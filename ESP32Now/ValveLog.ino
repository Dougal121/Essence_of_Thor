...
void WriteValveLogsToEEPROM(void){
  ...
  if ( hasRTC && bValveLogDirty ){
    ...
    snprintf(buff, BUFF_MAX, "%d/%02d/%02d", timeinfo.tm_year+1900, timeinfo.tm_mon+1, timeinfo.tm_mday);
...
int DayOfYear(time_t t)
{
  struct tm tm;
  localtime_r(&t, &tm);
  return tm.tm_yday + 1;   // 0-based → 1-based
}
...
//    esp_task_wdt_reset();    
    ...
    for ( j = 0 ; j < MAX_WEEKS ; j++){  //
      ...
      snprintf(buff, BUFF_MAX, "%02d/%02d/%04d - %02d/%02d/%04d", tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_mday, tm.tm_mon, tm.tm_year );     
