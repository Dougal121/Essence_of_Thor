...
//   now_t = now() ;
   for ( int i = 0 ; i < MAX_VALVE ; i++ ) {       // for each valve
    ...
    localtime_r(&myTime, &tm);
     if (( tm.tm_year > 2025 ) && ( myTime > ghks.AutoOff_t )){                     // dont do this if time is like ... BULLSHIT --switch off if time is crap (allow manual mode only
...
void UpdateATTGNew(time_t now_t){
int k ;  
time_t start_t , finish_t ;
//time_t now_t ;
time_t start_day_t ;
