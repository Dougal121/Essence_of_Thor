/*
    ...
    Yo Dude this is in need of ping test...

    Compile for TTGO LaRA board Version 1.0 
    1.9MB APP with OTA/190KB SPIFFS  - this is a custom setting you need to place this in the boards area of Arduino
    See https://www.hackster.io/dougal/beyond-essence-of-irrigation-part-2-lora-flavor-743704  for more details
...
//  dnsServer.start(53,"injector.local",myIP);
  ...
  randomSeed(now());                       // now we prolly have a good time setting use this to reroll the dice for reboot code
...
//    }
  ...
  if (rtc_hour != hour()){
    if (( bSentADCAlarmEmail ) && ( hour() == 0 )){  // reset the email alarm if still asserted
