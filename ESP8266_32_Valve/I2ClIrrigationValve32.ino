/*
    ...
    RTC DS3231  has 24C32 32K EEPROM
*/
...
const byte MAX_VALVE =  32 ;   // was 16    these two easily changed just watch the memory 
...
typedef struct __attribute__((__packed__)) {             // permanent record
  uint16_t Weekly[MAX_WEEKS];      //    53 x 2
  uint16_t Monthly[MAX_MONTHS];    //    12 x 2 
  uint16_t Daily[MAX_DAYS];        // Daily Totals  7 x 2
} valve_totals_t ;                 // 62 x 2 bytes ( 16x = 1024 )        144 bytes ( 16x = 2304 )
...
program_a         vp[MAX_VALVE];         // old program
...
typedef struct __attribute__((__packed__)) {
  ...
  cnc_v_t      cv[MAX_CNC] ;   // the most that we can have 
} cnc_t ; 
...
//    Serial.println(".local");
  ...
  server.on("/htmltest", handleHTMLTest);
...
//    }
    ...
    if ( bSendTestEmail ){
      SendEmailToClient(-1) ;                  // send test email 
  ...
  if ( rtc_min != minute()){   // once per minute stuff 
    bSendCtrlPacket = true ;
    lMinUpTime++ ;
